...
export async function GET() {
  ...
  const chaves = await db.apiKey.findMany({
    ...
    select: { id: true, nome: true, chavePrefixo: true, escopo: true, ativa: true, ultimoUsoEm: true, createdAt: true },
  })
...
export async function POST(req: NextRequest) {
  if (!exigirAdmin()) return NextResponse.json({ error: 'Acesso negado' }, { status: 403 })
  const { nome, escopo } = await req.json()
  ...
  const criada = await db.apiKey.create({
    data: { nome: nome.trim(), chaveHash: hashChaveApi(chave), chavePrefixo: prefixoVisivel(chave), escopo: escopoFinal },
  })
